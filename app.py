...
# ── COLORS ────────────────────────────────────────────────────────────────────
R   = "#CC0000"      # Jahn Rot
R2  = "#990000"      # Dunkleres Rot
R3  = "#3D0000"      # Sehr dunkles Rot (Card BG)
W   = "#FFFFFF"      # Weiß
W2  = "#F0F0F0"      # Off-White
W3  = "#CCCCCC"      # Hellgrau
...
section[data-testid="stSidebar"] + div::before {{
    ...
    background:linear-gradient(90deg, {R2} 0%, {R} 40%, {W} 60%, {R} 80%, {R2} 100%);
...
[data-testid="stSidebar"] {{
    background: #0A0A0A;
    border-right: 2px solid {R2};
}}
[data-testid="stSidebar"] label {{
    color: {MUT} !important; font-size:10px !important;
    letter-spacing:0.1em; text-transform:uppercase;
}}

/* Slider → Rot */
[data-testid="stSlider"] [role="progressbar"] {{ background:{R} !important; }}
[data-baseweb="slider"] [data-testid="stThumbValue"] {{ background:{R} !important; }}
...
.jcard {{
    ...
    border-top:3px solid {R}; border-radius:8px;
    padding:14px 12px; text-align:center; margin-bottom:4px;
}}
...
.div  {{ height:1px; background:linear-gradient(90deg,{R}55,{C2}); margin:10px 0; }}
</style>
...
B = dict(psv_med=29.45, hsr_bip=789.6, spr_bip=285.6, hi_bip=1086.3, expl_bip=1.32,
...
with st.sidebar:
    ...
    <div style="text-align:center;padding:20px 0 12px;">
        <img src="data:image/png;base64,{LOGO_B64}"
             style="width:100px;filter:drop-shadow(0 0 12px #CC000077);">
        <div style="font-size:15px;font-weight:800;color:#FFFFFF;
                    margin-top:10px;letter-spacing:0.06em;">JAHN REGENSBURG</div>
        <div style="font-size:10px;color:#666;letter-spacing:0.18em;
                    text-transform:uppercase;margin-top:3px;">Scouting · Wide Attacker</div>
    </div>
    <div class="div"></div>
    """, unsafe_allow_html=True)
    ...
    tw = w_rq+w_dr+w_bt
    st.success("Summe: 100% ✓") if tw==100 else st.warning(f"Summe: {tw}%")
...
with c_logo:
    st.markdown(f'''<img src="data:image/png;base64,{LOGO_B64}"
        style="width:52px;margin-top:6px;filter:drop-shadow(0 2px 8px #CC000066);">''',
        unsafe_allow_html=True)
with c_title:
    st.markdown(f'''
    <div style="padding-top:6px;">
        <span style="font-size:24px;font-weight:800;color:#FFFFFF;">Wide Attacker Scouting</span>
        <span style="font-size:13px;color:#666;margin-left:14px;">
            Jahn Regensburg &nbsp;·&nbsp; 7 Ligen &nbsp;·&nbsp;
            <span style="color:#CC0000;font-weight:600;">{len(df_f)} Spieler</span> nach Filter
        </span>
...
with tab1:
    st.markdown(f'<div style="font-size:11px;color:#555;font-family:DM Mono,monospace;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:8px;">{len(df_f)} Spieler · Sortiert nach {sort_col}</div>',
                unsafe_allow_html=True)
    if df_f.empty:
        ...
        def tier_bg(v):
            m = {"🔥 ELITE TARGET":"background-color:#3D0000;color:#FF9999;font-weight:700",
        ...
        def psv_bg(v):
            if pd.isna(v): return ""
            if v>=32:    return "background-color:#3D0000;color:#FF9999;font-weight:700"
            if v>=31:    return "background-color:#0D1F50;color:#90CAF9;font-weight:700"
            if v>=30.5:  return "background-color:#003344;color:#80DEEA;font-weight:700"
            if v>=29.45: return "background-color:#1F1000;color:#FFCC80;font-weight:700"
            return "color:#555"
        ...
        st.download_button("⬇️ Export CSV", csv, "jahn_scouting.csv", "text/csv")
...
with tab2:
    ...
    if df_f.empty:
        ...
        try:
            ...
            # Benchmark line — no annotation_font_color (compatibility fix)
            if x=="PSV-99":
            ...
            fig.update_layout(
                ...
                xaxis=dict(gridcolor="#1C1C1C", zeroline=False),
                yaxis=dict(gridcolor="#1C1C1C", zeroline=False),
                legend=dict(bgcolor="#161616", bordercolor="#1C1C1C", borderwidth=1),
...
**Speed Flag Cutoffs**
⚡ ELITE ≥32 · 🔵 HIGH ≥31 · 🟡 FAST ≥30.5 · 🟠 MEDIUM ≥29.45 · — darunter
        """)
    st.markdown("---")
